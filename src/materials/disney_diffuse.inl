Spectrum eval_op::operator()(const DisneyDiffuse& bsdf) const {
    ...
    Real FD_in = 1 + (FD_90 - 1) * pow(1 - abs(n_dot_in), 5);
    Real FD_out = 1 + (FD_90 - 1) * pow(1 - abs(n_dot_out), 5);
    Spectrum f_base_diffuse = (base_color / c_PI) * (FD_in * FD_out * n_dot_out); // ???
    // 2. f_subsurface
    Real FSS_90 = roughness * h_dot_out * h_dot_out;
