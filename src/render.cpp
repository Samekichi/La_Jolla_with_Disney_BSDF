...
#define FULL_IMAGE
//#define TOP_LEFT  // only render top-left corner 1/4 of the image, for efficient debugging
//#define TOP_RIGHT
//#define BOTTOM_LEFT
//#define BOTTOM_RIGHT

#ifndef RENDER_PERCENT_WIDTH
#define RENDER_PERCENT_WIDTH 0.33
#endif

#ifndef RENDER_PERCENT_HEIGHT
#define RENDER_PERCENT_HEIGHT 0.33
#endif

static void compute_offsets(int full_w, int full_h, int render_w, int render_h,
    int& offset_x, int& offset_y) {
#ifdef FULL_IMAGE
    offset_x = 0;
    offset_y = 0;
#elif defined(TOP_LEFT)
    offset_x = 0;
    offset_y = 0;
#elif defined(TOP_RIGHT)
    offset_x = full_w - render_w;
    offset_y = 0;
#elif defined(BOTTOM_LEFT)
    offset_x = 0;
    offset_y = full_h - render_h;
#elif defined(BOTTOM_RIGHT)
    offset_x = full_w - render_w;
    offset_y = full_h - render_h;
#else
    // detault to full image
    offset_x = 0;
    offset_y = 0;
#endif
}
...
Image3 aux_render(const Scene& scene) {
    int full_w = scene.camera.width, full_h = scene.camera.height;
    // compute render size in pixels
    int render_w = int(full_w * RENDER_PERCENT_WIDTH);
    int render_h = int(full_h * RENDER_PERCENT_HEIGHT);
    int offset_x, offset_y;
    compute_offsets(full_w, full_h, render_w, render_h, offset_x, offset_y);

    Image3 img(render_w, render_h);

    constexpr int tile_size = 16;
    int num_tiles_x = (render_w + tile_size - 1) / tile_size;
    int num_tiles_y = (render_h + tile_size - 1) / tile_size;

    parallel_for([&](const Vector2i& tile) {
        int x0 = tile[0] * tile_size;
        int x1 = std::min(x0 + tile_size, render_w);
        int y0 = tile[1] * tile_size;
        int y1 = std::min(y0 + tile_size, render_h);
        for (int y = y0; y < y1; y++) {
            for (int x = x0; x < x1; x++) {
                // global coordinates
                ...
                Ray ray = sample_primary(scene.camera,
                    Vector2((sample_x + Real(0.5)) / full_w,
                        (sample_y + Real(0.5)) / full_h));
                RayDifferential ray_diff = init_ray_differential(full_w, full_h);
                if (std::optional<PathVertex> vertex = intersect(scene, ray, ray_diff)) {
                    ...
                    if (scene.options.integrator == Integrator::Depth) {
                        ...
                        if (t != nullptr) {
                            ...
                            Real scaled_footprint = std::max(get_width(mipmap), get_height(mipmap)) *
                                std::max(t->uscale, t->vscale) * footprint;
                            Real level = log2(std::max(scaled_footprint, Real(1e-8)));
    ...
    parallel_for([&](const Vector2i& tile) {
        ...
        for (int y = y0; y < y1; y++) {
            for (int x = x0; x < x1; x++) {
                ...
                for (int s = 0; s < spp; s++) {
                    radiance += path_tracing(scene, sample_x, sample_y, rng);
                }
    ...
    parallel_for([&](const Vector2i& tile) {
        ...
        for (int y = y0; y < y1; y++) {
            for (int x = x0; x < x1; x++) {
                ...
                int spp = scene.options.samples_per_pixel;
                for (int s = 0; s < spp; s++) {
                    Spectrum L = f(scene, sample_x, sample_y, rng);
                    if (isfinite(L)) {
                        radiance += L;
                    }
                }
...
// Default: Render Full-size Image
